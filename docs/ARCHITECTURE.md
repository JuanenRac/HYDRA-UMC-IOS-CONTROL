...
- `POST /api/login` - returns a bearer token, required for every write
  below and for the WebSocket upgrade. No account or password is
  pre-filled by this app. A development server seeds an `admin`/`admin`
  account; a production first start instead requires explicit
  `HYDRA_UMC_BOOTSTRAP_ADMIN_USERNAME`/`HYDRA_UMC_BOOTSTRAP_ADMIN_PASSWORD`
  credentials (see HYDRA-UMC-SERVER's own `users.ts`, which backs a real
  multi-user store). Additional lower-privilege "operator" accounts can be
  created from Config > Users in the browser UI.
...
lib/
├── main.dart
├── app_version.dart   (generated by tool/bump_version.dart)
├── l10n/          app_*.arb (7 languages) + generated app_localizations*.dart, language_prefs.dart
├── models/        server_info.dart, hydra_state.dart, assistant_reply.dart
├── network/       hydra_api_client.dart, hydra_websocket.dart, discovery.dart,
│                  auth_prefs.dart, biometric_helper.dart, state_cache.dart
├── state/         robot_view_model.dart, hydra_error.dart
└── ui/            login_screen.dart, biometric_gate_screen.dart, main_screen.dart,
                   dashboard_screen.dart, control_screen.dart, camera_screen.dart,
                   three_d_screen.dart, telemetry_screen.dart, settings_screen.dart,
                   voice_assistant_dialog.dart, widgets/
```
